...
public class GPUImageFilterGroup extends GPUImageFilter {
    ...
    public void onInit() {
        ...
        for (GPUImageFilter filter : mFilters) {
            filter.setmInputIsVideo(this.ismInputIsVideo());
            filter.init();
        }
    }
    ...
    public void onDraw(final int targetTexture, final int textureId, final FloatBuffer cubeBuffer,
        ...
        if (mMergedFilters != null) {
            ...
            for (int i = 0; i < size; i++) {
                ...
                //Here is where camera rotation for the second filter happens
                if (i == 0) {
                    filter.onDraw(targetTexture, previousTexture, cubeBuffer, textureBuffer);
                } else if (i == size - 1) {
                    filter.setInputRotation(90);
                    filter.onDraw(targetTexture, previousTexture, mGLCubeBuffer, (size % 2 == 0) ? mGLTextureFlipBuffer : mGLTextureBuffer);

                } else {
                    filter.onDraw(targetTexture, previousTexture, mGLCubeBuffer, mGLTextureBuffer);
                }
